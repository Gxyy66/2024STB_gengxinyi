...
#표분의 크기2로 추출된 데이터의 평균을 구한 뒤 막대그래프 작성
...
print(meanofz2)
barplot(table(z2),xlab="평균2",ylab="도수",main="표분크기2의 도수분포포")

#sample size=3
com3=combn(data50,3)#데이터에서 3개씩 선택하며 데이터 생성
com3t=t(com3)#데이터의 전치
z3=rowMeans(com3t)#전치된 데이터의 행평균 계산
meanofz3=mean(z3)#기댓값을 계산하며 저장
print(meanofz3)
barplot(table(z3),xlab="평균3",ylab="도수",main="표분크기3의 도수분포포")

#sample size=5
com5=combn(data50,5)#데이터에서 5개씩 선택하며 데이터 생성
com5t=t(com5)#데이터의 전치
z5=rowMeans(com5t)#전치된 데이터의 행평균 계산
meanofz5=mean(z5)#기댓값을 계산하며 저장
print(meanofz5)
barplot(table(z5),xlab="평균3",ylab="도수",main="표분크기5의 도수분포포")
...
library(sampling)
sam2=strata(data=iris,stratanames=c("Species"),size=c(3,3,3),method='srswor')#iris데이터 사용하며 표분추출,species층을 기준으로
